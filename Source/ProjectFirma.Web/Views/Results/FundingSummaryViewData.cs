    ...
    public class FundingSummaryViewData : FirmaUserControlViewData
    {
        ...
        public FundingSummaryViewData()
        {
            ...
            if (fundingSourceCustomAttributeType != null)
            {
                ...
                TotalAwarded = Math.Round(projects.Sum(x => x.GetTargetedFundingForFundingSources(awardFundingSourceIDs) ?? 0));
                TotalMatched = Math.Round(projects.Sum(x => x.GetTargetedFundingForFundingSources(matchedFundingSourceIDs) ?? 0));
