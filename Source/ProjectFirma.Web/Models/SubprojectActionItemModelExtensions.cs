    ...
    public static class SubprojectActionItemModelExtensions
    {
        ...
        public static HtmlString GetEditButtonHtmlString(this SubprojectActionItem actionItem, bool userHasEditPermissions)
        {
            var buttonString = userHasEditPermissions 
                ? $"{DhtmlxGridHtmlHelpers.MakeEditIconAsModalDialogLinkBootstrap(new ModalDialogForm(actionItem.GetEditUrl(), ModalDialogFormHelper.DefaultDialogWidth, $"Edit {FieldDefinitionEnum.SubprojectActionItem.ToType().FieldDefinitionDisplayName}"))}" 
                : string.Empty;
