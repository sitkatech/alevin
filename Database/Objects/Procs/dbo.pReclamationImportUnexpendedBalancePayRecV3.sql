...
-- TK 12/12/2024 - After more discussion, we have changed this to check to allow a null posting date if the UnexpendedBalance is 0.
declare @nullSplDateCount int
set @nullSplDateCount = (select count(*) from ImportFinancial.ImportFinancialImpPayRecUnexpendedV3 as pr where PostingDatePerSpl is null and UnexpendedBalance > 0)
if (@nullSplDateCount > 0)
begin
   raiserror('Expected no records with a null PostingDatePerSpl and a value greater than 0 in UnexpendedBalance in ImportFinancial.ImportFinancialImpPayRecUnexpendedV3. But found some.', 16,1)
   return -1
end
